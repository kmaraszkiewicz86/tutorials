...
using CocurrentProgramming.CoreModels;

namespace CocurrentProgramming
{
    ...
    class Program
    {
        ...
        static CocumberExampleClass cocumberExmapleClass = new CocumberExampleClass();

        static void Main(string[] args)
        {
            var listOPfThreads = new List<Task>();

            for (var index = 0; index < 100; index++)
            {
                var task = cocumberExmapleClass.Write(index).ContinueWith((t) =>
                {
                    Console.WriteLine(cocumberExmapleClass.Read());
                });

                listOPfThreads.Add(task);
            }

            foreach (var task in listOPfThreads)
            {
                task.Wait();
            }

            //var listOPfThreads = new List<Thread>();

            //for (var index = 0; index < 100; index++)
            //{
            //    var thread = new Thread((data) => OnWrok((int)data));
            //    thread.Start(index);
            //}

            Console.WriteLine("=========================================");
            Console.WriteLine(listOPfThreads.Count);
        }
    }
}
